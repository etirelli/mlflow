...
## Trace archival (workspace override)

- The server-wide archive repository location is set with `--trace-archival-location` (or `MLFLOW_TRACE_ARCHIVAL_LOCATION`). When workspaces are enabled, a workspace can override this by setting `trace_archival_location` on the workspace (e.g. `mlflow.create_workspace(..., trace_archival_location="s3://bucket/traces")` or the Create/Update Workspace REST API). Archived trace span data for that workspace is then stored under the workspace-specific archive repository location.
